...
def get_model():
    ...
    model.compile(optimizer=Adam(lr, decay=10^-6), loss='binary_crossentropy', metrics=['accuracy'])
...
for gamma in [0.6, 0.8, 1.0]:
    ...
    for i in range(n_trials):
        ...
        #Execute TF-IDF on data
        from sklearn.feature_extraction.text import TfidfVectorizer
        tfidf = TfidfVectorizer(max_features=2000)
        X_train_tfidf = np.array(tfidf.fit_transform(X).todense())
        X_test_tfidf = np.array(tfidf.transform(x_test).todense())

        #Split data on source and target tasks
        x_train_s = X_train_tfidf[:ms, :]
        y_train_s = Y[:ms]

        x_train_t = X_train_tfidf[ms:m, :]
        y_train_t = Y[ms:m]
    ...
    #Fix f
        ...
        y_pred_fix_f = model_fix_f.predict(X_test_tfidf)
    ...
    #Fine tune f
        ...
        model_finetune_f.compile(optimizer=Adam(lr, decay=10^-6), loss='binary_crossentropy', metrics=['accuracy'])
        model_finetune_f.fit(x_train_t, y_train_t_binary, validation_split=0.1, epochs=nb_epoch, batch_size=batch_size, verbose=2)
        y_pred_finetune_f = model_finetune_f.predict(X_test_tfidf)
